...
let shimeji = (function(){
    return {
        spritesheets,
        activeSprite:"my-hero-academia-katsuki-bakugo-kacchan-by-superevey",
        spawnIndex:1,
        mouse:[0,0],
        buds:[],
        ...
        spawn:function(x=100,y=100,sprite=this.activeSprite){
        ...
        cleanActions:function(bud){
            switch(bud.action){
                ...
                case "dragging":
                    ...
                    break
                case "split":
                    this.spawn(bud.x+10,bud.y,bud.sprite)
                    break
            }
        },
        ...
        setup:async function(storage={}){
            this.spritesheets = await this.spritesheets
            
            if("autoSpawnIndex" in storage)
                this.activeSprite = storage["autoSpawnIndex"]
            if(!("autoSpawn" in storage))
                this.spawn()
            else if(storage["autoSpawn"])
                this.spawn()
...
browser.storage.sync.get(["autoSpawn","autoSpawnIndex"], function(items) {
    shimeji.setup(items)
});
...
browser.runtime.onMessage.addListener((req)=>{
    switch(req.type){
        ...
        case "clear":
            ...
            break
        case "changeSprite":
            shimeji.activeSprite = req.payload
            break
    }
})
